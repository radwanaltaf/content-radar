...
/**
 * Blank out the configured identifiers before spell-checking, so a schema.org
 * type or a CSS property does not read as a misspelling. Case sensitive, so
 * "Organization" the identifier is exempt and "organization" the word is not.
 */
function exemptions(config, text) {
  let out = text;
  for (const term of config.vocabulary.spellingExceptions ?? []) {
...
export function check(config) {
  ...
  for (const entry of live(all)) {
    ...
    // Markdown tables that open with "| |" render an empty <th>, which is a
    // real screen-reader problem: the column has no accessible name, so every
    // row header in it is announced without context. The syntax makes this the
    // path of least resistance, which is why it is checked rather than
    // remembered.
    for (const line of prose(entry, { keepLinks: true }).split('\n')) {
      if (!/^\s*\|/.test(line)) continue;
      const cells = line.split('|').slice(1, -1);
      if (cells.length < 2) continue;
      if (/^[\s:-]*$/.test(cells.join(''))) continue; // the --- separator row
      const emptyLeading = cells[0].trim() === '';
      if (emptyLeading) {
        errors.push(
          `${entry.rel}: a table row starts with an empty cell, which renders an empty <th> ` +
            `and leaves that column with no accessible name. Give the first column a header. ` +
    ...
    if (config.rules.auditStatistics) {
      const found = [];
      for (const paragraph of prose(entry, { keepLinks: true }).split(/\n\s*\n/)) {
  ...
  for (const dir of config.vocabulary.alsoSweep) {
    for (const file of walk(path.join(config.root, dir))) {
      ...
      if (config.vocabulary.spelling === 'en-GB') {
        for (const [wrong, right] of AMERICANISMS) {
          if (word(wrong).test(exemptions(config, text))) {
