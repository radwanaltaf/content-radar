...
/**
 * Body with the things that are not prose removed: fenced code, inline code,
 * and the target half of a markdown link.
 *
 * Link targets matter more than they look. A URL slug is somebody else's
 * spelling, and citing a real source should never fail a spelling rule. This
 * came up on the first article that cited a Gartner press release, whose slug
 * contains "canceled". Keeping the link text and dropping the target is the
 * only correct reading: the text is ours and the URL is not.
 */
export function prose(entry, { keepLinks = false } = {}) {
  const withoutCode = entry.body.replace(/```[\s\S]*?```/g, '').replace(/`[^`]*`/g, '');
  // The statistics audit needs the targets, because its whole question is
  // whether a figure has a link near it. Stripping them there made every
  // figure report as uncited, which is a check that still runs and has stopped
  // saying anything. Hence the option rather than one behaviour.
  if (keepLinks) return withoutCode;
  return withoutCode.replace(/\]\([^)]*\)/g, ']').replace(/\bhttps?:\/\/\S+/g, '');
}
